<?php if(isset($this->penalità)): ?>
	...
	<div id="operazioni-other" class="column last">
		<ul class="operazioni-content">
			...
			<?php else: ?>
				<li class="column last"><a class="undo-punteggi-active column last operazione" href="<?php echo $this->linksObj->getLink('dettaglioGiornata',array('giorn'=>$this->giornPrec,'squad'=>$this->idSquadra)); ?>">Indietro di una giornata</a></li>
			<?php endif; ?>
			<?php if(!$this->giornSucc): ?>
				<li class="simil-link redo-punteggi-unactive column last">Avanti di una giornata</li>
			<?php else: ?>
			<li class="column last"><a class="redo-punteggi-active column last operazione" href="<?php echo $this->linksObj->getLink('dettaglioGiornata',array('giorn'=>$this->giornSucc,'squad'=>$this->idSquadra)); ?>">Avanti di una giornata</a></li>
			<?php endif; ?>
		</ul>
	</div>
